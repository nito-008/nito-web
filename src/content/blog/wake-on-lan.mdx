...
いよいよ本命の遠隔起動機能を実装します。遠隔起動にはWake-on-Lan(WoL)という仕組みを使います。WoLはLAN内のPCの電源を別のPCから遠隔でONにする機能のことです。今回はハードウェアWoLといって、完全に電源が落ちている状態からの起動なので、有線LANでつながっている必要があります。
